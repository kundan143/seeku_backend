-- Sidebar menu entry for the Conductor & Insulation Design quick-calculator screen,
-- placed as a sibling of the existing "Production Datasheet" page (inherits the same
-- parent menu and icon so it shows up in the Design & Costing group), appended after
-- all existing siblings.
INSERT INTO menu_master (parent_id, menu_name, link, icon, parent_rank, child_rank)
SELECT sibling.parent_id, 'Conductor & Insulation Design', '/design-and-costing/conductor-insulation-design', sibling.icon, sibling.parent_rank,
...
AND NOT EXISTS (SELECT 1 FROM menu_master WHERE link = '/design-and-costing/conductor-insulation-design');
...
JOIN menu_master mm ON mm.link = '/design-and-costing/conductor-insulation-design'
WHERE NOT EXISTS (
