-- Sidebar menu entry for the new Salary Increment page (/hr/salary-increment) - previously the
-- only way to give an increment or view increment history was via icons buried in Employee Salary
-- Master's table; this gives it a direct route/menu entry of its own. Placed as a sibling of
-- "Employee Salary Master" (same HR group), inheriting its parent/icon.
INSERT INTO menu_master (parent_id, menu_name, link, icon, parent_rank, child_rank)
SELECT sibling.parent_id, 'Salary Increment', '/hr/salary-increment', 'pi pi-arrow-up-right', sibling.parent_rank,
       (SELECT COALESCE(MAX(child_rank), 0) + 1 FROM menu_master WHERE parent_id = sibling.parent_id)
FROM menu_master sibling
WHERE sibling.link = '/hr/employee-salary-master'
AND NOT EXISTS (SELECT 1 FROM menu_master WHERE link = '/hr/salary-increment');

-- Copy each user's existing Employee Salary Master permission level onto the new menu item as a default.
...
JOIN menu_master sibling ON sibling.id = mp.menu_id AND sibling.link = '/hr/employee-salary-master'
