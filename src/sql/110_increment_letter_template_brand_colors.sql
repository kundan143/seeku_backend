-- Restyles the default Increment Letter template (originally inserted by 108) to match
-- the company logo's navy + red palette instead of the generic navy/gray it shipped with.
-- Only touches the row we ourselves inserted by name — if HR has since renamed or customized
-- it via the PDF Template Master UI, this update matches nothing and leaves it alone.
