const { orgCategoriesMaster } = require("../models");
...
exports.addData = async function (body) {
	try {
		var result = await orgCategoriesMaster.create(body.data);
...
exports.updateData = async function (body) {
	try {
		await orgCategoriesMaster.update(body.data, {
			where: {
				id: body.id
			}
		});
...
exports.deleteData = async function (body) {
	try {
		await orgCategoriesMaster.destroy({
			where: {
				id: body.id
			}
		});
...
exports.getAllData = async function () {
	try {
		var data = await orgCategoriesMaster.findAll({
			order: [
				['id', 'ASC']
			]
		});
...
exports.getCatCreditDebitNote = async function (is_cd_note) {
	try {
		var data = await orgCategoriesMaster.findAll({
			where: {
				is_cd_note: is_cd_note
			}
		});
		responseCodes.SUCCESS.data = data;
		responseCodes.SUCCESS.message = "";
