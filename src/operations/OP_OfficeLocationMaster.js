const { officeLocationMaster, cityMaster, stateMaster } = require("../models");
const { responseCodes } = require("../services/baseReponse");

exports.addData = async function (body) {
	try {
		var result = await officeLocationMaster.create(body.data);
...
exports.updateData = async function (body) {
	try {
		await officeLocationMaster.update(body.data, {
			where: {
				id: body.id
			}
		});
...
exports.getAllData = async function () {
	try {
		var data = await officeLocationMaster.findAll({
			include: [
				{ model: cityMaster, attributes: ["name"] },
				{ model: stateMaster, attributes: ["name"] },
			],
			where: {
				status: 1
			},
