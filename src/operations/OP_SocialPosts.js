const { socialPosts, socialPostLikes, socialPostComments } = require("../models");
...
exports.getAllData = async function ({ limit, offset, search, requesterId }) {
  try {
    const query = `
      SELECT sp.id, sp.content, sp.image_path, sp.share_count, sp.created_date,
...
exports.getComments = async function (post_id) {
  try {
    const query = `
      SELECT spc.id, spc.post_id, spc.content, spc.created_date, spc.modified_date,
